...
- **Verify agent** (separate thread, after the member merged `<user>` →
  `dev`) proves the integrated lane on that member's live dev server:
  the changed surface, deploy logs, OpenObserve alerts/traces for the
  window. It is read-only — `LIVE-OK <sha> <url>` or `LIVE-FAIL <sha>`
  + the failing signal; fixes route back to a worker. Do not let the
  verify agent edit code.
