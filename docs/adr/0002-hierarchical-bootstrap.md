...
Windows x86_64 is native — `.\setup.ps1` runs a PowerShell twin of the
same module catalog (ADR 0012); WSL2 Ubuntu remains an optional POSIX
path, Windows arm64 is fail-closed. bun/npm/brew copies are not
uninstalled; PATH order after `. install/env.sh` (or `env.ps1`) prefers
the pinned binary.
