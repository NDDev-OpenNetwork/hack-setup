...
### State 2026-09-21 — devin-setup twin (Devin CLI harness)

- `devin-setup/` is a self-contained second setup for the Devin CLI:
  same law→runtime→proof loop, same `--member`/`--os` flags, same
  module mechanism. Orchestration = herdr panes (not codex_app threads);
  cmux hosts the outer layout.
- Law: `devin-setup/build/devin-pin.json` — devin `3000.10.31`
  (versioned `static.devin.ai/cli/<ver>/setup.{sh,ps1}`, no published
  sha — URL is the version), herdr `0.9.1` sha256 assets ×5 platforms,
  session law, MCP, hooks, plugin list. `build/stack-pin.json` inside
  is a generated byte-copy of the root pin (`sync_stack_pin.py`).
- Session law: `DEVIN_PERMISSION_MODE=bypass` env (no config key),
  `subagents_enabled=false`, `auto_update=false`, `read_config_from`
  all 5 false, model `swe-2-max` — managed block in user config via
  `repair_devin_setup.py` (single writer, JSONC merge, .hack-bak).
- `.devin/` projection: config.json, mcp_config.json (same 6 servers;
  keenable `${KEENABLE_API_KEY}` header), hooks.v1.json (7 events) →
  `devin_mode.py` (Devin protocol port of hack_mode.py; lane guard
  honors `.devin/lanes.json` AND `.codex/lanes.json`).
- Modules: 10/15/30 byte-identical to `install/` (shared-sync checker);
  `20-devin-cli` (user config, devin, herdr, integration, python3 shim
  on Windows), `40-devin-verify` (`devin plugins install --local`).
- Plugin `hack-devin-workflow`: 6 skills; `herdr-handoff` covers worker
  spawn via `herdr agent start/prompt/read/wait` + durable session close.
